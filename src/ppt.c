...
void findRepeats(SeqStructPtr seqP, char** repeats, int numRepeats) {
	...
	while (error_rate < max_error) {
			...
			// In case last nucletide was a mismatch.
                ...
                error_count++;
                error_rate = (float) error_count / (float) (purine + 1);
                i++;
        }
        i--; // To counter overshoot by mismatcher.
	    ...
	    while (error_rate < max_error) {
            ...
            fprintf(outf, "%s, %s, %d, %d, %d, %f, %d", base_string, sequence, length, start, end, ratioGC, error_count - 1);
...
int main(int argc, char* argv[])
{
   ...
   if (argc < 2)
	...
	fprintf(stderr,"\t-e <error rate>\t- the maximum mismatch rate allowed (default = 0.1)\n");
 ...
 /* parse arguments */
   for(i = 1; i < argc; i++)
   {
    if(argv[i][0] == '-')
    {
        ...
        {
            i++;
            if (sscanf(argv[i], "%f", &max_error) != 1) exit(2);
        }
